...
impl LiquidEngine {
    ...
    // ---- Direct Position/Velocity Control (for free drag) ----
    ...
    pub fn set_node_velocity(&mut self, id: u32, vx: f32, vy: f32) {
        let i = id as usize;
        if i < self.world.max_entities && self.world.alive[i] {
    ...
    /// Set the text label for a node. The label is rendered inside the data box.
    pub fn set_node_label(&mut self, id: u32, label: &str) {
        self.world.set_label(id as usize, label);
    }

    pub fn get_node_position(&self, id: u32) -> f32 {
        // Returns packed: x in upper 32 bits via a trick.
        // Actually, we'll return x and expose a second method for y.
        // wasm-bindgen doesn't support tuples, so we use two calls.
        let i = id as usize;
        if i < self.world.max_entities && self.world.alive[i] {
            self.world.pos_x[i]
        } else {
    ...
    pub fn get_node_position_y(&self, id: u32) -> f32 {
        let i = id as usize;
        if i < self.world.max_entities && self.world.alive[i] {
            self.world.pos_y[i]
        } else {
...
mod bench_tests {
    ...
    fn bench_10k_nodes_300_frames() {
        ...
        // Data boxes with bitmap font rendering are ~10x more pixels than circles.
        // Realistic target for 10K boxes on CPU in debug mode: <300ms/frame.
        assert!(ms_per_frame < 300.0, "Performance target: < 300ms/frame for 10K boxes, got {:.2}ms", ms_per_frame);
    }
    ...
    fn bench_3k_nodes_baseline() {
        ...
        // Data boxes: ~8x fill cost vs circles. Realistic for 3K boxes.
